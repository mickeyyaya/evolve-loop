...
# AC-ID:         cycle-5-009
# Description:   domain-phase-catalog.md §5 wave-status table: Ops row flipped from "⬜ queued" to ✅ done (cycle 5)
# Evidence:      docs/architecture/domain-phase-catalog.md wave status table
# Author:        tdd-engineer
# Created:       2026-06-06T00:00:00Z
# Acceptance-of: scout-report.md AC#9 — wave-ops-tdd-and-phases
...
row=$(grep -E '^\| *Ops *\|' "$CATALOG" | head -1)
if [ -z "$row" ]; then
  echo "RED: Ops row missing from wave-status table" >&2; exit 1
fi
# Negative: must no longer be queued.
if echo "$row" | grep -q '⬜'; then
  echo "RED: Ops row still queued: $row" >&2; exit 1
fi
# Positive: must be marked done.
if ! echo "$row" | grep -q '✅'; then
  echo "RED: Ops row not marked ✅ done: $row" >&2; exit 1
fi

echo "GREEN: Ops wave status flipped to done" >&2
exit 0
