...
# AC-ID:         cycle-6-011
# Description:   domain-phase-catalog.md §5 wave-status table: PM row flipped from "⬜ queued" to ✅ done (cycle 6)
# Evidence:      docs/architecture/domain-phase-catalog.md wave status table
# Author:        tdd-engineer
# Created:       2026-06-06T00:00:00Z
# Acceptance-of: scout-report.md AC#1 — wave-pm-status-flip
...
row=$(grep -E '^\| *PM *\|' "$CATALOG" | head -1)
if [ -z "$row" ]; then
  echo "RED: PM row missing from wave-status table" >&2; exit 1
fi
# Negative: must no longer be queued.
if echo "$row" | grep -q '⬜'; then
  echo "RED: PM row still queued: $row" >&2; exit 1
fi
# Positive: must be marked done.
if ! echo "$row" | grep -q '✅'; then
  echo "RED: PM row not marked ✅ done: $row" >&2; exit 1
fi

echo "GREEN: PM wave status flipped to done" >&2
exit 0
