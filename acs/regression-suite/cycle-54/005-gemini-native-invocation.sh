...
# Fixture capabilities file with non_interactive_prompt=true.
# Required since cycle-55 flipped production cap to false; this seam keeps the
# predicate testing the NATIVE path regardless of the production capability value.
FIXTURE_CAP="$TMP_DIR/gemini-fixture.capabilities.json"
printf '{"adapter":"gemini","version":2,"supports":{"non_interactive_prompt":true}}' > "$FIXTURE_CAP"
...
EVOLVE_TESTING=1 \
  ...
  EVOLVE_GEMINI_CAP_FILE="$FIXTURE_CAP" \
