...
# AC-ID:         cycle-10-011
# Description:   domain-phase-catalog.md §5 wave-status table: Product row flipped from "⬜ queued" to ✅ done (cycle 10)
...
row=$(grep -E '^\| *Product *\|' "$CATALOG" | head -1)
if [ -z "$row" ]; then
  echo "RED: Product row missing from wave-status table" >&2; exit 1
fi
# Negative: must no longer be queued.
if echo "$row" | grep -q '⬜'; then
  echo "RED: Product row still queued: $row" >&2; exit 1
fi
# Positive: must be marked done.
if ! echo "$row" | grep -q '✅'; then
  echo "RED: Product row not marked ✅ done: $row" >&2; exit 1
fi

echo "GREEN: Product wave status flipped to done" >&2
exit 0
