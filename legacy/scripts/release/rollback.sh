...
#        EVOLVE_BYPASS_SHIP_VERIFY=1 bash legacy/scripts/lifecycle/ship.sh "revert: ..."
...
#
# Usage:
#   bash legacy/scripts/release/rollback.sh <journal.json> [--reason "..."] [--dry-run]
#
...
if [ "$DRY_RUN" = "1" ]; then
    ...
    log "DRY-RUN: would EVOLVE_BYPASS_SHIP_VERIFY=1 bash legacy/scripts/lifecycle/ship.sh \"revert: $REASON [rollback of v$VERSION]\""
