...
# form `bash legacy/scripts/dispatch/subagent-run.sh <agent> <cycle> <workspace>` — every other
# command falls through with ALLOW.
#
# Decision tree:
#   1. Read JSON payload from stdin → extract command.
#   2. If command does not match `bash legacy/scripts/dispatch/subagent-run.sh ...` → ALLOW.
...
if [ "$TOOL_NAME" = "Agent" ]; then
    if [ -f "$CYCLE_STATE_FILE" ] && [ -s "$CYCLE_STATE_FILE" ]; then
        deny "Agent tool forbidden during evolve-loop cycles — use \`bash legacy/scripts/dispatch/subagent-run.sh <agent> <cycle> <workspace>\` so the kernel ledger captures dispatch"
    else
...
# ---- Trigger detection -----------------------------------------------------
# Match `bash legacy/scripts/dispatch/subagent-run.sh ...` (with optional ./, /full/path/, and
# leading whitespace). Anything else: passthrough.

TRIMMED="${COMMAND#"${COMMAND%%[![:space:]]*}"}"
case "$TRIMMED" in
    bash*legacy/scripts/dispatch/subagent-run.sh*|sh*legacy/scripts/dispatch/subagent-run.sh*|*/subagent-run.sh*)
...
case "$REQUESTED_AGENT" in
    intent|scout|builder|auditor|evaluator|inspirer|orchestrator|retrospective|tdd-engineer|plan-reviewer|triage|memo|build-planner)
        DERIVED_ROLE="$REQUESTED_AGENT"
        ;;
    intent-worker-*|scout-worker-*|builder-worker-*|auditor-worker-*|evaluator-worker-*|inspirer-worker-*|retrospective-worker-*|tdd-engineer-worker-*|plan-reviewer-worker-*|triage-worker-*|memo-worker-*|build-planner-worker-*)
...
case "$PHASE" in
    ...
    tdd)          EXPECTED="tdd-engineer build-planner builder orchestrator" ;;
    build-planner) EXPECTED="build-planner builder orchestrator" ;;
...
if [ "$REQUESTED_AGENT" = "scout" ] && command -v jq >/dev/null 2>&1; then
    ...
    if [ "$INTENT_REQUIRED" = "true" ]; then
        ...
        if [ -z "$INTENT_ENTRY" ]; then
            deny "intent_required=true (cycle $CYCLE_ID) but no intent ledger entry found; run \`bash \$EVOLVE_PLUGIN_ROOT/legacy/scripts/dispatch/subagent-run.sh intent $CYCLE_ID $WORKSPACE_PATH\` before scout"
        fi
...
if [ "${EVOLVE_REQUIRE_TEAM_CONTEXT:-0}" = "1" ] && [ "$REQUESTED_AGENT" = "builder" ]; then
    ...
    if [ -z "$WORKSPACE_PATH" ]; then
        ...
        TEAM_CTX_SH="$EVOLVE_PLUGIN_ROOT/legacy/scripts/utility/team-context.sh"
