#!/usr/bin/env bash
#
# ship-integration-test.sh — End-to-end tests for legacy/scripts/lifecycle/ship.sh.
#
...
#
# Usage: bash legacy/scripts/ship-integration-test.sh
...
RESOLVE_ROOTS_SH="$REPO_ROOT/legacy/scripts/lifecycle/resolve-roots.sh"  # v8.18.0: ship.sh sources this
...
cd "$REPO"
set +e
bash legacy/scripts/lifecycle/ship.sh "test commit" >/tmp/ship-out 2>&1
RC=$?
set -e
...
git branch -M main
set +e
bash legacy/scripts/lifecycle/ship.sh "feat: test" >/tmp/ship-out 2>&1
RC=$?
set -e
if [ "$RC" = "0" ]; then
...
git branch -M main
set +e
bash legacy/scripts/lifecycle/ship.sh "feat: shipping with WARN" >/tmp/ship-out 2>&1
RC=$?
set -e
...
seed_audit "$REPO" "WARN"
set +e
EVOLVE_STRICT_AUDIT=1 bash legacy/scripts/lifecycle/ship.sh "should not ship" >/tmp/ship-out 2>&1
RC=$?
set -e
...
set +e
bash legacy/scripts/lifecycle/ship.sh "should refuse" >/tmp/ship-out 2>&1
RC=$?
set -e
...
set +e; bash legacy/scripts/lifecycle/ship.sh "first ship" >/tmp/ship-out 2>&1; RC1=$?; set -e
# Modify ship.sh — simulates tampering. plugin.json:version stays at 1.0.0.
echo "# malicious comment" >> legacy/scripts/lifecycle/ship.sh
# Second ship: same version, different SHA → REAL tampering → rc=2.
echo "another change" > another.txt
seed_audit "$REPO" "PASS"
set +e; bash legacy/scripts/lifecycle/ship.sh "second ship" >/tmp/ship-out 2>&1; RC2=$?; set -e
...
git branch -M main
set +e
EVOLVE_BYPASS_SHIP_VERIFY=1 bash legacy/scripts/lifecycle/ship.sh "emergency" >/tmp/ship-out 2>&1
RC=$?
set -e
if [ "$RC" = "0" ]; then
...
# release class is for legacy/scripts/release-pipeline.sh use; skips audit-binding
...
git branch -M main
set +e
bash legacy/scripts/lifecycle/ship.sh --class release "release: v9.99.99" >/tmp/ship-out 2>&1
RC=$?
set -e
...
git branch -M main
set +e
bash legacy/scripts/lifecycle/ship.sh --class manual "manual change" </dev/null >/tmp/ship-out 2>&1
RC=$?
set -e
...
git branch -M main
set +e
EVOLVE_SHIP_AUTO_CONFIRM=1 bash legacy/scripts/lifecycle/ship.sh --class manual "ci change" </dev/null >/tmp/ship-out 2>&1
RC=$?
set -e
...
git branch -M main
set +e
EVOLVE_BYPASS_SHIP_VERIFY=1 bash legacy/scripts/lifecycle/ship.sh "legacy bypass" </dev/null >/tmp/ship-out 2>&1
RC=$?
set -e
...
cd "$REPO"
set +e
bash legacy/scripts/lifecycle/ship.sh --class garbage "msg" </dev/null >/tmp/ship-out 2>&1
RC=$?
set -e
...
git branch -M main
set +e
bash legacy/scripts/lifecycle/ship.sh "feat: ship with exit-1" >/tmp/ship-out 2>&1
RC=$?
set -e
if [ "$RC" = "0" ]; then
...
git branch -M main
set +e
bash legacy/scripts/lifecycle/ship.sh "feat: ship with exit-2" >/tmp/ship-out 2>&1
RC=$?
set -e
...
git branch -M main
set +e
bash legacy/scripts/lifecycle/ship.sh "feat: ship with verdict fail" >/tmp/ship-out 2>&1
RC=$?
set -e
...
EOF
set +e
bash legacy/scripts/lifecycle/ship.sh "ship dual-verdict" </dev/null >/tmp/ship-out 2>&1
RC=$?
set -e
...
seed_audit "$REPO" "PASS"
set +e; bash legacy/scripts/lifecycle/ship.sh "first ship at v1.0.0" >/tmp/ship-out 2>&1; RC1=$?; set -e
# Bump plugin version AND modify ship.sh to simulate plugin update
echo '{"version":"1.1.0"}' > "$REPO/.claude-plugin/plugin.json"
echo "# v1.1.0 ship.sh tweak" >> legacy/scripts/lifecycle/ship.sh
echo "second" > q2.txt
seed_audit "$REPO" "PASS"
set +e; bash legacy/scripts/lifecycle/ship.sh "ship at v1.1.0" >/tmp/ship-out 2>&1; RC2=$?; set -e
...
ACTUAL=$(sha256 "$REPO/legacy/scripts/lifecycle/ship.sh")
...
echo "audited" > r.txt
seed_audit "$REPO" "PASS"
set +e; bash legacy/scripts/lifecycle/ship.sh "ship after migration" >/tmp/ship-out 2>&1; RC=$?; set -e
...
git branch -M main
set +e; bash legacy/scripts/lifecycle/ship.sh "feat: cycle 1 work" >/tmp/ship-out 2>&1; RC=$?; set -e
...
git branch -M main
set +e
EVOLVE_SHIP_AUTO_CONFIRM=1 bash legacy/scripts/lifecycle/ship.sh --class manual "manual: ad-hoc fix" </dev/null >/tmp/ship-out 2>&1
RC=$?
set -e
...
git branch -M main
set +e
bash legacy/scripts/lifecycle/ship.sh "feat: claims do not match" >/tmp/ship-out 2>&1
RC=$?
set -e
...
git branch -M main
set +e
bash legacy/scripts/lifecycle/ship.sh --class release "release: v9.0.0" >/tmp/ship-out 2>&1
RC=$?
set -e
last_msg=$(git -C "$REPO" log -1 --format='%B')
