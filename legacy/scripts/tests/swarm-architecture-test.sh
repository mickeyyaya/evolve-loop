...
if grep -q '"plan-review"' "$REPO_ROOT/legacy/scripts/lifecycle/cycle-state.sh"; then
...
SR="$REPO_ROOT/legacy/scripts/dispatch/subagent-run.sh"
...
bash legacy/scripts/dispatch/subagent-run.sh dispatch-parallel scout 99000 "$WS" >/dev/null 2>&1 || rc=$?
