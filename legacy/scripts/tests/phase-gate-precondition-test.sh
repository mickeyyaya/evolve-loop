...
# legacy/scripts/guards/phase-gate-precondition.sh (v8.13.1).
#
# Tests: trigger detection (only `bash legacy/scripts/dispatch/subagent-run.sh`), per-phase
# expected-agent allowlist, re-spawn handling, no-cycle passthrough, bypass.
#
# Usage: bash legacy/scripts/phase-gate-precondition-test.sh
...
GATE="$REPO_ROOT/legacy/scripts/guards/phase-gate-precondition.sh"
...
# === Test 2: bash legacy/scripts/lifecycle/ship.sh → ALLOW (not our trigger) ===================
header "Test 2: bash legacy/scripts/lifecycle/ship.sh → ALLOW (passthrough)"
set_state build builder
expect_allow "ship.sh invocation" '{"tool_input":{"command":"bash legacy/scripts/lifecycle/ship.sh \"feat: x\""}}'

# === Test 3: no cycle-state → ALLOW (ad-hoc invocation) =======================
header "Test 3: no cycle-state → ALLOW"
rm -f "$TEST_STATE"
expect_allow "ad-hoc subagent-run" \
    '{"tool_input":{"command":"bash legacy/scripts/dispatch/subagent-run.sh scout 99 .evolve/runs/cycle-99"}}'

# === Test 4: phase=calibrate, agent=scout → ALLOW =============================
header "Test 4: phase=calibrate + scout → ALLOW"
set_state calibrate ""
expect_allow "calibrate→scout" \
    '{"tool_input":{"command":"bash legacy/scripts/dispatch/subagent-run.sh scout 99000 .evolve/runs/cycle-99000"}}'
...
expect_deny "calibrate→builder" \
    '{"tool_input":{"command":"bash legacy/scripts/dispatch/subagent-run.sh builder 99000 .evolve/runs/cycle-99000"}}'

# === Test 6: phase=build, agent=auditor → ALLOW ===============================
header "Test 6: phase=build + auditor → ALLOW (next phase)"
set_state build builder
expect_allow "build→auditor" \
    '{"tool_input":{"command":"bash legacy/scripts/dispatch/subagent-run.sh auditor 99000 .evolve/runs/cycle-99000"}}'
...
expect_allow "audit→retrospective" \
    '{"tool_input":{"command":"bash legacy/scripts/dispatch/subagent-run.sh retrospective 99000 .evolve/runs/cycle-99000"}}'
...
expect_allow "unknown agent 'foobar'" \
    '{"tool_input":{"command":"bash legacy/scripts/dispatch/subagent-run.sh foobar 99000 .evolve/runs/cycle-99000"}}'
...
expect_allow "research→scout-worker-codebase" \
    '{"tool_input":{"command":"bash legacy/scripts/dispatch/subagent-run.sh scout-worker-codebase 99000 .evolve/runs/cycle-99000"}}'
...
expect_allow "audit→auditor-worker-eval" \
    '{"tool_input":{"command":"bash legacy/scripts/dispatch/subagent-run.sh auditor-worker-eval 99000 .evolve/runs/cycle-99000"}}'
...
expect_allow "research→scout-worker-research" \
    '{"tool_input":{"command":"bash legacy/scripts/dispatch/subagent-run.sh scout-worker-research 99000 .evolve/runs/cycle-99000"}}'
...
expect_allow "build→dispatch-parallel auditor" \
    '{"tool_input":{"command":"bash legacy/scripts/dispatch/subagent-run.sh dispatch-parallel auditor 99000 .evolve/runs/cycle-99000"}}'
...
expect_deny "build→dispatch-parallel scout" \
    '{"tool_input":{"command":"bash legacy/scripts/dispatch/subagent-run.sh dispatch-parallel scout 99000 .evolve/runs/cycle-99000"}}'
...
expect_deny "intent→build-planner (recognized but out-of-phase)" \
    '{"tool_input":{"command":"bash legacy/scripts/dispatch/subagent-run.sh build-planner 99000 .evolve/runs/cycle-99000"}}'
