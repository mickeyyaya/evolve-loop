...
Direct CLI invocations (e.g., `bash legacy/scripts/dispatch/evolve-loop-dispatch.sh ...`) remain opt-in via `EVOLVE_REQUIRE_INTENT=1` — only the slash-command path defaults intent on. This keeps existing test infrastructure unchanged.
