...
Stagnation detection is handled by `legacy/scripts/observability/cycle-health-check.sh` (deterministic). Read stagnation findings from `$WORKSPACE/cycle-health.json`. If 3+ patterns active → recommend HALT.
