...
- **Dynamic Turn Limits (Marginal Value Gating):** If a task requires multiple retries (audit iterations > 2) and the `tokenEfficiency` is poor, compute the marginal value: `expected_improvement = completion_probability_delta * task_value`. If `expected_improvement < turn_cost` (token expenditure), emit a **Marginal Value HALT**. It is better to fail the task than to waste the perCycle token budget on diminishing returns.
