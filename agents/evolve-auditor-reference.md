...
1. **Validate predicates**: predicates are Go tests (`//go:build acs`) under `go/acs/{cycle<N>,regression,redteam}/predicates_test.go` — validated by `go vet -tags acs` + the `acssuite` compile-error hard-gate. (Bash predicates were fully retired in the EGPS Go-native migration; ADR-0042.)
2. **Run suite**: `evolve acs suite --cycle "$cycle"`. This deterministic host-side Go runner executes three scopes, each as a separate `go test -json -tags acs` and merges them into one `acs-verdict.json`: the **current cycle** (`./acs/cycle<N>`), the curated **regression** set (`./acs/regression/<sub>`, run every cycle), and the standing **red-team** anti-gaming predicates (`./acs/redteam`, run every cycle). A non-compiling predicate package is a HARD error, never a silent PASS. (Successor to the bash `run-acs-suite.sh` of ADR-0025.)
3. **Cross-check**: Every AC MUST have a predicate (a Go test func, per the AC-Materialization Contract).
