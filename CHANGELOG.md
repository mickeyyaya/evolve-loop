...
- **Token-budget cost calculation + all cost gates.** The dollar-cost calculation was unreliable across LLM models (tmux/subscription claude reports `$0`, gemini used a hardcoded price table, ollama is free), so every decision keyed on cost behaved differently per model. Removed: the `internal/budget` package; the phase-level cost guard (`CostGuardDecorator`); the `evolve loop` batch-cap stop (`rc=4`/`stop_reason=batch_cap`) and budget-driven completion (`rc=0`/`stop_reason=budget`); the cost-percentage checkpoint trigger; the router `budget-exhausted` optional-phase skip plus `BudgetEnvelope`/`PhaseRequest.Budget`; the cyclehealth `cost_envelope` anomaly; the `subagent.QuotaLikely` cost-crossed heuristic; and the gemini per-token pricing + claude budget-tier resolution/enforcement in the shell adapters.

### Changed

- Cost is now **display-only telemetry**: `total_cost_usd` in loop output and per-phase `cost_usd` are still reported (passthrough of what each CLI emits) but no longer gate anything.
- Budget flags/env are **deprecated no-ops** (accepted but ignored; `StatusDead` in the flag registry): `EVOLVE_MAX_BUDGET_USD`, `EVOLVE_BATCH_BUDGET_CAP`, `EVOLVE_BATCH_BUDGET_DISABLE`, `EVOLVE_BUILDER_COST_THRESHOLD`, `EVOLVE_BUILDER_COST_GUARD_STRICT`, `EVOLVE_BUDGET_CAP`, `EVOLVE_BUDGET_ENFORCE`, `EVOLVE_BUDGET_MAX_CYCLES`, `EVOLVE_PHASE_COST_CEILING`, `EVOLVE_CHECKPOINT_WARN_AT_PCT`, `EVOLVE_CHECKPOINT_AT_PCT`, and the `--budget-usd` / `--budget` / `--batch-cap-usd` CLI flags. Use `--cycles N` to bound a run.
