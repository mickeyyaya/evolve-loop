...
# EXIT CODES
#
#    0 — all cycles ran and verified
#    1 — runtime failure (missing prerequisite, bad arg, run-cycle.sh failed)
#    2 — verification failure (a cycle did not produce all 3 ledger entries)
...
verify_cycle() {
    ...
    if [ "$s" -lt 1 ] || [ "$b" -lt 1 ] || [ "$a" -lt 1 ]; then
        log "VERIFY-FAIL: cycle $cycle pipeline incomplete (scout=$s builder=$b auditor=$a) — orchestrator shortcut detected"
        return 2
    fi
    return 0
}
...
for ((i=1; i<=CYCLES; i++)); do
    ...
    if [ "${EVOLVE_DISPATCH_VERIFY:-1}" = "1" ]; then
        if ! verify_cycle "$ran_cycle"; then
            log "verification failed — see above; aborting batch"
            DISPATCH_RC=2
            break
        fi
    else
...
if [ "$DISPATCH_RC" = "0" ]; then
    ...
    log "DONE-WITH-VERIFICATION-FAILURE: a cycle bypassed Scout/Builder/Auditor"
