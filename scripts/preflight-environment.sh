...
# USAGE
#
#   bash scripts/preflight-environment.sh              # probe + emit JSON to stdout
#   bash scripts/preflight-environment.sh --write      # also persist to .evolve/environment.json
#   bash scripts/preflight-environment.sh --summary    # print human-readable summary instead of JSON
#   bash scripts/preflight-environment.sh --help
#
# ENV
#
...
if [ -x "$EVOLVE_PLUGIN_ROOT/scripts/detect-nested-claude.sh" ]; then
    NESTED_RESULT=$(bash "$EVOLVE_PLUGIN_ROOT/scripts/detect-nested-claude.sh" 2>/dev/null || echo "standalone")
else
