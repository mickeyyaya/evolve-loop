...
cmd_run() {
    ...
    [[ "$agent_role" =~ ^(scout|tdd-engineer|builder|auditor|inspirer|evaluator|retrospective|orchestrator|plan-reviewer|intent|triage|memo|code-simplifier|code-reviewer)$ ]] || fail "unknown agent: $agent"
...
cmd_dispatch_parallel() {
    ...
    [[ "$agent" =~ ^(scout|tdd-engineer|builder|auditor|inspirer|evaluator|retrospective|orchestrator|plan-reviewer|intent|triage|memo)$ ]] \
