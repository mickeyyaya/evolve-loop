...
```bash
cd ~/ai/claude/evolve-loop/go && go test ./internal/core/... -run TestIsTransientBridgeError -count=1 -timeout 30s -v 2>&1 | grep -E "--- PASS|--- FAIL|\[no tests to run\]"
```
...
```bash
cd ~/ai/claude/evolve-loop/go && go test ./internal/core/... -run TestOrchestrator_RetryOnTransientExit -count=1 -timeout 30s -v 2>&1 | grep -E "--- PASS|--- FAIL|\[no tests to run\]"
```
...
```bash
cd ~/ai/claude/evolve-loop/go && go test ./internal/core/... -run TestOrchestrator_NonCanonicalVerdictRetry -count=1 -timeout 30s -v 2>&1 | grep -E "--- PASS|--- FAIL|\[no tests to run\]"
```
...
```bash
cd ~/ai/claude/evolve-loop/go && go test ./internal/core/... -run TestOrchestrator_NonTransientError_NoRetry -count=1 -timeout 30s -v 2>&1 | grep -E "--- PASS|--- FAIL|\[no tests to run\]"
```
...
```bash
cd ~/ai/claude/evolve-loop/go && go test ./internal/core/... -run TestTransientRetry_Exhausted_WritesFailureDiag -count=1 -timeout 30s -v 2>&1 | grep -E "--- PASS|--- FAIL|\[no tests to run\]"
```
...
```bash
cd ~/ai/claude/evolve-loop/go && go test ./internal/core/... -run TestFAILVerdict_NotRetried -count=1 -timeout 30s -v 2>&1 | grep -E "--- PASS|--- FAIL|\[no tests to run\]"
```
Expected: `--- PASS: TestFAILVerdict_NotRetried` (no "no tests to run" line)

### AC-7: All existing core tests still pass [code]
```bash
cd ~/ai/claude/evolve-loop/go && go test ./internal/core/... -timeout 90s -count=1 2>&1 | grep -E "^ok |^FAIL "
```
Expected: lines starting with `ok` only (no FAIL lines)

### AC-8: self-healing-gaps.md marks GAP 1 and GAP 5 as DONE [code]
```bash
grep -c "DONE" ~/ai/claude/evolve-loop/docs/architecture/self-healing-gaps.md
```
...
```bash
grep -c "ErrTransientBridgeFailure" ~/ai/claude/evolve-loop/go/internal/core/errors.go
```
Expected: at least 1
...
```bash
cd ~/ai/claude/evolve-loop/go && go test ./internal/core/... -run TestOrchestrator_PhaseArtifactTimeout_RetriesAndRecovers -count=1 -timeout 30s -v 2>&1 | grep -E "--- PASS|--- FAIL"
```
...
```bash
cd ~/ai/claude/evolve-loop/go && go test ./internal/core/... -run TestTransientRetry_LedgerEntry -count=1 -timeout 30s -v 2>&1 | grep -E "--- PASS|--- FAIL|\[no tests to run\]"
```
