...
```bash
cd ~/ai/claude/evolve-loop/go && go test ./internal/core/... -run TestOrchestrator_Backfill_LedgerEntry -count=1 -timeout 30s -v 2>&1 | grep -E "--- PASS|--- FAIL|\[no tests to run\]"
```
...
```bash
grep -c "phase-timing" ~/ai/claude/evolve-loop/CLAUDE.md
```
Expected: at least 1 (entry in the output/diagnostics subsystem table)

### AC-3: CLAUDE.md documents failure-diag.json [code]
```bash
grep -c "failure-diag" ~/ai/claude/evolve-loop/CLAUDE.md
```
Expected: at least 1

### AC-4: docs/architecture/phase-timing-and-diagnostics.md exists [code]
```bash
test -f ~/ai/claude/evolve-loop/docs/architecture/phase-timing-and-diagnostics.md && echo "OK"
```
Expected: OK

### AC-5: phase-timing-and-diagnostics.md covers phase-timing.json format [code]
```bash
grep -c "duration_ms" ~/ai/claude/evolve-loop/docs/architecture/phase-timing-and-diagnostics.md
```
Expected: at least 1

### AC-6: phase-timing-and-diagnostics.md covers failure-diag.json format [code]
```bash
grep -c "failure-diag" ~/ai/claude/evolve-loop/docs/architecture/phase-timing-and-diagnostics.md
```
...
```bash
grep -cE "phase_retry|kind.*backfill|backfill.*kind" ~/ai/claude/evolve-loop/docs/architecture/phase-timing-and-diagnostics.md
```
Expected: at least 2
...
```bash
cd ~/ai/claude/evolve-loop/go && go test ./internal/core/... -run TestOrchestrator_Backfill_NoLedgerEntryWhenDisabled -count=1 -timeout 30s -v 2>&1 | grep -E "--- PASS|--- FAIL|\[no tests to run\]"
```
...
```bash
cd ~/ai/claude/evolve-loop/go && go test ./internal/core/... ./internal/backfill/... -timeout 90s -count=1 2>&1 | grep -E "^ok |^FAIL "
```
...
```bash
cd ~/ai/claude/evolve-loop/go && go test ./internal/core/... -run TestOrchestrator_Backfill_LedgerRole -count=1 -timeout 30s -v 2>&1 | grep -E "--- PASS|--- FAIL|\[no tests to run\]"
```
