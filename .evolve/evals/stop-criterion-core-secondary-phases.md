# Eval: stop-criterion-core-secondary-phases
...
- `[code]` `cd go && go test ./... -count=1 -timeout 120s 2>&1 | tail -5 | grep -v "^$" | grep -qv "FAIL"`

## Acceptance Checks

- `[code]` `grep -A3 "STOP CRITERION" agents/evolve-tdd-engineer.md | grep -qi "gate\|complete\|done\|write\|halt\|stop"`
- `[code]` `grep -A3 "STOP CRITERION" agents/evolve-adversarial-review.md | grep -qi "gate\|complete\|done\|write\|halt\|stop"`
