...
func (e *Engine) doctorDeep(ctx context.Context, cli, binary string) DeepProbe {
	...
	rc, err := e.deps.Runner(cctx, binary, "", probeArgs, driverEnv(e.deps, nil), nil, &out, &out)
...
func doctorVersion(ctx context.Context, deps Deps, binary string) string {
	var out bytes.Buffer
	if _, err := deps.Runner(ctx, binary, "", []string{"--version"}, driverEnv(deps, nil), nil, &out, &out); err != nil {
		return "unknown"
	}
	line := out.String()
