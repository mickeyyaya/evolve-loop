...
func TestDossierFailure_FailCarriesIdentity(t *testing.T) {
	...
	if err := writeCycleDossier(nil, cycleDossierParams{ProjectRoot: root, WorkspacePath: ws, Cycle: 9, Goal: "fix Z", RunID: "run9", Outcome: VerdictFAIL}); err != nil {
...
func TestDossierFailure_ReasonsTruncatedAndCapped(t *testing.T) {
	...
	if err := writeCycleDossier(nil, cycleDossierParams{ProjectRoot: root, WorkspacePath: ws, Cycle: 10, Goal: "fix W", RunID: "run10", Outcome: VerdictFAIL}); err != nil {
		t.Fatalf("writeCycleDossier: %v", err)
	}
...
func TestDossierFailure_PassKeepsShape(t *testing.T) {
	...
	if err := writeCycleDossier(nil, cycleDossierParams{ProjectRoot: root, WorkspacePath: ws, Cycle: 12, Goal: "improve U", RunID: "run12", Outcome: CycleOutcomeShippedViaBuild}); err != nil {
		t.Fatalf("writeCycleDossier: %v", err)
	}
