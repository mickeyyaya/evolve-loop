...
func TestDossierFailure_FailCarriesIdentity(t *testing.T) {
	...
	if err := writeCycleDossier(nil, root, ws, 9, "fix Z", "run9", VerdictFAIL, nil, nil, nil, nil); err != nil {
...
func TestDossierFailure_ReasonsTruncatedAndCapped(t *testing.T) {
	...
	if err := writeCycleDossier(nil, root, ws, 10, "fix W", "run10", VerdictFAIL, nil, nil, nil, nil); err != nil {
		t.Fatalf("writeCycleDossier: %v", err)
	}
...
func TestDossierFailure_AbsentArtifactsDegrade(t *testing.T) {
	root := t.TempDir()
	initDossierRepo(t, root)
	if err := writeCycleDossier(nil, root, t.TempDir(), 11, "fix V", "run11", VerdictFAIL, nil, nil, nil, nil); err != nil {
...
func TestDossierFailure_PassKeepsShape(t *testing.T) {
	...
	if err := writeCycleDossier(nil, root, ws, 12, "improve U", "run12", CycleOutcomeShippedViaBuild, nil, nil, nil, nil); err != nil {
		t.Fatalf("writeCycleDossier: %v", err)
	}
