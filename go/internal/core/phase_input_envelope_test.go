...
// envelope ONLY at advisory+ with the planner enabled and a readable file;
// every other case returns "" so the phase reads disk as before (byte-identical
// dispatch at off/shadow).
func TestReadUpstreamBuildPlan(t *testing.T) {
	ws := t.TempDir()
	if err := os.WriteFile(filepath.Join(ws, "build-plan.md"), []byte("PLAN"), 0o644); err != nil {
		t.Fatal(err)
	}
	planner := map[string]string{"EVOLVE_BUILD_PLANNER": "1"}
	...
	for _, c := range []struct {
		...
		{"advisory+build+planner+empty file → empty (phase falls back to disk, byte-identical)", config.StageAdvisory, PhaseBuild, planner, wsEmpty, ""},
		{"advisory+build+planner but empty workspace → empty", config.StageAdvisory, PhaseBuild, planner, "", ""},
	} {
		if got := readUpstreamBuildPlan(c.stage, c.phase, c.env, c.ws); got != c.want {
