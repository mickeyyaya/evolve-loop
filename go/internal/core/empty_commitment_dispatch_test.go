...
func TestEnforceNext_EmptyTriageCommitmentIsALegalTermination(t *testing.T) {
	...
	cases := []struct {
		...
		{"empty-commitment-terminates-even-when-ship-planned", PhaseTriage, PhaseTDD, 0, true, PhaseEnd, true},
		// The pre-existing no-ship early exit is unaffected.
		{"empty-commitment-terminates-when-no-ship", PhaseTriage, PhaseTDD, 0, false, PhaseEnd, true},
		...
		{"committed-work-outranks-a-spurious-end-proposal", PhaseTriage, PhaseTDD, 1, true, PhaseTDD, false},
		{"two-committed-tasks-still-advance", PhaseTriage, PhaseTDD, 2, true, PhaseTDD, false},
		...
		{"unknown-commitment-stays-blocked-when-ship-planned", PhaseTriage, PhaseTDD, -1, true, PhaseTDD, false},
		...
		{"scout-edge-unchanged", PhaseScout, PhaseTriage, -1, true, PhaseTriage, false},
		...
		{"post-build-never-terminates-early", PhaseBuild, PhaseAudit, 0, true, PhaseAudit, false},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			sig := digestTriageWorkspace(t, tc.committed)
			gotPhase, gotOK := o.enforceNext(tc.current, tc.staticNext, sig,
