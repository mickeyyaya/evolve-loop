...
func TestWriteCycleDossier_AcquiresGitMutationLock(t *testing.T) {
	...
	if err := writeCycleDossier(spy.acquire, root, t.TempDir(), 11, "wire the lock", "run", CycleOutcomeShippedViaBuild, nil, nil, nil, nil); err != nil {
...
func TestWriteCycleDossier_ConcurrentLanesEachAcquireAndRelease(t *testing.T) {
	...
	for i := 0; i < lanes; i++ {
		...
		go func(i int, root string) {
			defer wg.Done()
			errs[i] = writeCycleDossier(spy.acquire, root, t.TempDir(), 100+i, "lane", "run", CycleOutcomeShippedViaBuild, nil, nil, nil, nil)
		}(i, root)
	}
	wg.Wait()
...
func TestWriteCycleDossier_LockError_FailsOpen(t *testing.T) {
	...
	if err := writeCycleDossier(spy.acquire, root, t.TempDir(), 12, "fail open", "run", CycleOutcomeShippedViaBuild, nil, nil, nil, nil); err != nil {
...
func TestWriteCycleDossier_ConcurrentRealRepo_BothLand(t *testing.T) {
	...
	for i := 0; i < 2; i++ {
		...
		go func(i int) {
			defer wg.Done()
			errs[i] = writeCycleDossier(defaultGitMutationLock, root, t.TempDir(), 200+i, "concurrent", "run", CycleOutcomeShippedViaBuild, nil, nil, nil, nil)
		}(i)
	}
	wg.Wait()
