...
func TestWriteCycleDossier_WritesValidArtifact(t *testing.T) {
	...
	ws := t.TempDir()
	if err := writeCycleDossier(nil, root, ws, 7, "improve X", "run-ulid", CycleOutcomeShippedViaBuild, nil, nil, nil, nil); err != nil {
...
func TestWriteCycleDossier_FailOutcomeRecordsDefect(t *testing.T) {
	root := t.TempDir()
	initDossierRepo(t, root)
	if err := writeCycleDossier(nil, root, t.TempDir(), 8, "fix Y", "run2", VerdictFAIL, nil, nil, nil, nil); err != nil {
		t.Fatalf("writeCycleDossier: %v", err)
	}
...
func TestWriteCycleDossier_LeavesCleanTree(t *testing.T) {
	root := t.TempDir()
	initDossierRepo(t, root)
	if err := writeCycleDossier(nil, root, t.TempDir(), 537, "closeout", "run3", CycleOutcomeShippedViaBuild, nil, nil, nil, nil); err != nil {
		t.Fatalf("writeCycleDossier: %v", err)
	}
