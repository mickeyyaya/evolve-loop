...
func TestModule_ClosedSet(t *testing.T) {
	...
	for _, m := range []Module{ModuleOrchestrator, ModuleAdvisor, ModuleRunner, ModuleBridge, ModuleLiveness, ModuleShip,
		...
		ModuleInbox, ModuleConfig, ModuleLoop, ModuleWatchdog, ModuleObserver, ModuleDashboard, ModuleLedger, ModuleOutcome, ModuleFailureDiag, ModuleSignalCenter} {
		if !m.Known() {
	...
	if got := Modules(); len(got) != 24 || !sortedModules(got) {
		t.Errorf("Modules() must list the 24 declared modules sorted, got %v", got)
	}
}
...
func TestKind_ClosedSetAndTerminal(t *testing.T) {
	...
	all := []Kind{KindPhaseDispatched, KindPhaseOutcome, KindPhaseAborted, KindGateRejected, KindGateCorrected,
		...
		KindPaneLiveness, KindLedgerAppended, KindOutcomeWarning, KindFailureDiagWarning, KindGatePassed, KindCycleSealed, KindLoopWave, KindLoopHalt, KindLoopEscalation,
