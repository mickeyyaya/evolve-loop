...
func TestModule_ClosedSet(t *testing.T) {
	...
	for _, m := range []Module{ModuleOrchestrator, ModuleAdvisor, ModuleRunner, ModuleBridge, ModuleLiveness, ModuleShip,
		...
		ModuleInbox, ModuleConfig, ModuleLoop, ModuleWatchdog, ModuleObserver, ModuleDashboard, ModuleLedger, ModuleOutcome, ModuleSignalCenter} {
		if !m.Known() {
	...
	if got := Modules(); len(got) != 23 || !sortedModules(got) {
		t.Errorf("Modules() must list the 23 declared modules sorted, got %v", got)
	}
}
