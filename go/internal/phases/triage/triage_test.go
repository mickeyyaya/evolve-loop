...
`
	...
	clock := fixedClock(time.Unix(1_700_000_000, 0), 80*time.Millisecond)
	phase := New(Config{
		Bridge:  fb,
...
func TestRun_PopulatesExtraFlagsFromProfile(t *testing.T) {
	root := writeTriageProfile(t, `{"extra_flags":["--require-full"],"permission_mode":"acceptEdits"}`)
	fb := &fakeBridge{}
	phase := New(Config{Bridge: fb, Prompts: fakePromptsFS("body")})
	_, _ = phase.Run(context.Background(), core.PhaseRequest{
...
// TestRun_EnvOverridesProfilePermissionMode — EVOLVE_TRIAGE_PERMISSION_MODE
// in req.Env beats the profile's permission_mode.
func TestRun_EnvOverridesProfilePermissionMode(t *testing.T) {
	root := writeTriageProfile(t, `{"permission_mode":"acceptEdits"}`)
	fb := &fakeBridge{}
	phase := New(Config{Bridge: fb, Prompts: fakePromptsFS("body")})
	_, _ = phase.Run(context.Background(), core.PhaseRequest{
		Cycle: 1, ProjectRoot: root, Workspace: t.TempDir(),
		Env: map[string]string{"EVOLVE_TRIAGE_PERMISSION_MODE": "plan"},
	})
	got := strings.Join(fb.gotReq.ExtraFlags, " ")
