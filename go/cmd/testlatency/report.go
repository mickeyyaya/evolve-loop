...
type Report struct {
	...
	// Incomplete names packages that emitted test events but no terminal
	// pass/fail/skip summary — i.e. Wall is unknown (a truncated stream, a
	// panic, or a build/timeout kill). Surfaced as a report note so a partial
	// run isn't silently mistaken for a complete one.
	Incomplete []string
}
...
func Parse(r io.Reader) (*Report, error) {
	...
	for sc.Scan() {
		...
		case "run", "start":
			...
			// Incomplete rather than vanishing entirely.
			if e.Package != "" {
				get(e.Package)
			}
		case "pass", "fail", "skip":
