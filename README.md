...
**Locked-down / corporate environment?** For an air-gapped install of a pre-approved binary (no compiler, no network) verified against its published SHA256 fingerprint — `sh install.sh --binary <artifact> --checksums checksums.txt` — see [docs/operations/corporate-deployment.md](docs/operations/corporate-deployment.md).
...
**Current (v22.18)** — full release history in [CHANGELOG.md](CHANGELOG.md). Releases are cut via `evolve release X.Y.Z`.
