...
RUN_CYCLE="$EVOLVE_PLUGIN_ROOT/archive/legacy/scripts/dispatch/run-cycle.sh"
[ -f "$RUN_CYCLE" ] || RUN_CYCLE="$EVOLVE_PROJECT_ROOT/archive/legacy/scripts/dispatch/run-cycle.sh"
...
if [ "$bump_rc" -ne 0 ]; then
    if [ "$bump_rc" = "2" ]; then
        ...
        log "    bash archive/legacy/scripts/dispatch/resume-cycle.sh"
