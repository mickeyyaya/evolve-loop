...
#
# Usage:
#   bash archive/legacy/scripts/dispatch/run-cycle.sh [GOAL]
#   bash archive/legacy/scripts/dispatch/run-cycle.sh --cycle 8200 [GOAL]
#   bash archive/legacy/scripts/dispatch/run-cycle.sh --dry-run   # print what would happen without spawning
#   bash archive/legacy/scripts/dispatch/run-cycle.sh --simulate  # walk every phase via cycle-simulator.sh (no LLM)
#
# Lifecycle:
       ...
       [ -x "$EVOLVE_PROJECT_ROOT/legacy/scripts/observability/render-cli-resolution.sh" ]; then
        ...
        if [ -n "$_cli_section" ]; then
            ...
            awk '
                ...
                !skip { print }
            ' "$WORKSPACE/orchestrator-report.md" > "$_orep_tmp"
            printf '\n%s\n' "$_cli_section" >> "$_orep_tmp"
            mv -f "$_orep_tmp" "$WORKSPACE/orchestrator-report.md" 2>/dev/null || rm -f "$_orep_tmp"
