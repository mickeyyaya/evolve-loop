...
#   2. Loops `bash archive/legacy/scripts/dispatch/run-cycle.sh "<goal>"` once per cycle.
...
# USAGE
#
#   bash archive/legacy/scripts/dispatch/evolve-loop-dispatch.sh [CYCLES] [STRATEGY] [GOAL...]
#   bash archive/legacy/scripts/dispatch/evolve-loop-dispatch.sh --dry-run [args...]
#   bash archive/legacy/scripts/dispatch/evolve-loop-dispatch.sh --help
#
# ARGS
#
...
RUN_CYCLE="${RUN_CYCLE_OVERRIDE:-$EVOLVE_PLUGIN_ROOT/archive/legacy/scripts/dispatch/run-cycle.sh}"
...
case "$EVOLVE_PROJECT_ROOT" in
    */plugins/cache/*|*/plugins/marketplaces/*)
        ...
        log "         Or:  EVOLVE_PROJECT_ROOT=/path/to/project bash <plugin>/archive/legacy/scripts/dispatch/evolve-loop-dispatch.sh ..."
        exit 10
        ;;
esac
...
if [ "$RESUME_MODE" = "1" ]; then
    RESUME_SCRIPT="$EVOLVE_PLUGIN_ROOT/archive/legacy/scripts/dispatch/resume-cycle.sh"
...
for ((i=1; i<=CYCLES; i++)); do
    ...
    if [ "$rc" -ne 0 ]; then
        ...
        if [ -f "$cycle_state_file" ] && command -v jq >/dev/null 2>&1; then
            ...
            if [ "$cp_enabled" = "true" ] && [ "$cp_reason" = "quota-likely" ]; then
                ...
                log "  to resume manually: bash archive/legacy/scripts/dispatch/resume-cycle.sh"
                DISPATCH_RC=5
    ...
    if [ "$BATCH_BUDGET_DISABLE" != "1" ] && command -v bc >/dev/null 2>&1 \
        ...
        if [ "$_pct" -ge "$CHECKPOINT_AT_PCT" ] && [ "${EVOLVE_CHECKPOINT_REQUEST:-0}" != "1" ]; then
            ...
            log "  to resume after this batch ends: bash archive/legacy/scripts/dispatch/evolve-loop-dispatch.sh --resume"
...
if [ "$DISPATCH_RC" = "0" ]; then
    ...
    log "  → to resume manually right now: bash archive/legacy/scripts/dispatch/resume-cycle.sh"
