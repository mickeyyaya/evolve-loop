...
# delegates to `bridge launch --cli=codex`. Bridge picks up
...
if [[ "${EVOLVE_USE_BRIDGE:-1}" != "0" ]] && command -v bridge >/dev/null 2>&1; then
  ...
  if [[ "$_bridge_schema" == "1" ]]; then
    ...
    _bridge_tier=$(bridge --json probe --cli=codex 2>/dev/null | jq -r '.results[0].tier // "none"' 2>/dev/null || echo "none")
    if [[ "$_bridge_tier" != "none" && "$_bridge_tier" != "null" ]]; then
      echo "[codex] delegating to bridge launch --cli=codex (tier=$_bridge_tier; default-on; EVOLVE_USE_BRIDGE=0 to disable)" >&2
      exec bridge launch --cli=codex
    else
      echo "[codex] WARN: bridge installed but cli=codex tier=$_bridge_tier (manifest missing or binary not on PATH); falling back to native adapter" >&2
    fi
  else
    echo "[codex] WARN: bridge installed but schema_version='$_bridge_schema' (expected 1); falling back to native adapter" >&2
  fi
