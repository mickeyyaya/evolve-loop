...
|--------------------|--------------|-----------------------------|
| `scripts/observability/cycle-health-check.sh` | 11 integrity signals (velocity, artifacts, ledger, tokens) | **Yes** — velocity anomaly, missing artifacts, missing ledger entries |
| `scripts/verification/verify-eval.sh` | Re-runs eval graders independently | **Yes** — no evals existed to run |
...
### Implementation: `scripts/lifecycle/phase-gate.sh` (301 lines)
...
- [scripts/lifecycle/phase-gate.sh](../scripts/lifecycle/phase-gate.sh) — the remediation script (301 lines)
