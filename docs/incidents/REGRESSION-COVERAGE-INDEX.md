...
| [cycles 1634/1636 — a phase's own FAIL reason persisted nowhere; the seal called it "phase-infra class"](2026-09-13-phase-own-fail-reason-invisible.md) | the C1 record (`recovery.PhaseOutcome` → `phasetiming.Entry`) had no field for the phase's own diagnostics, so `phaseOutcomeFrom` dropped them; only floor phases had a side channel — triage's protected-surface admission rejection (a reasoned Classify FAIL) sealed as "verdict FAIL with no recorded abort reason (phase-infra class)", cyclehealth said only "recorded verdict FAIL", nothing logged; two investigations, two poison items kept being claimed | `core/failure_learning.go` (phaseOutcomeFrom, recordPhaseOutcome), `core/failreasons_backfill.go`, `core/cyclerun.go` (verdictFailReason), `cyclestate/result.go` (ErrorMessages + severity vocabulary), `phasetiming/phasetiming.go`, `recovery/outcome.go`, `cyclehealth/outcome.go` (reads through phasetiming.Entry) | ✅ | `core/phase_diagnostics_seal_test.go::TestRunCycle_TriageOwnFailReasonReachesTheSealAndTheRecord` (cycles 1634/1636's shape through the composed `RunCycle` path; RED on the old code with the exact live marker) + `::TestRecordPhaseOutcome_CarriesThePhaseDiagnosticsAndNamesAReasonedFail` (the chokepoint relays, records and logs once) + `core/failreasons_backfill_test.go::TestBackfillFailReasons_PhaseOwnDiagnosticsNameTheReason` (priority rule; warnings are not reasons) + `phasetiming/diagnostics_test.go::TestEntry_DiagnosticsRideTheRecordAndAreOmittedWhenAbsent` (wire format, legacy logs parse to nil) + `cyclehealth/outcome_diagnostics_test.go::TestClassifyOutcome_FailVerdictDetailNamesThePhaseOwnReason` + `cyclestate/diagnostic_test.go::TestErrorMessages_ProjectsOnlyErrorSeverityInOrder` (the one projection) + the composed test's `ClassifyOutcome` parity assertion (writer and reader share the schema home); 8 mutants killed, each confirmed to build: relay dropped, entry omits, log never fires, seal ignores, formatter drops messages, projection counts warnings (8 tests across 3 packages), cyclehealth ignores the projection, cyclehealth reads a missing record as present |
