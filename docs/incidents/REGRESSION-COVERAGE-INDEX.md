...
| [cycle-1630 — a no-work cycle labelled `SHIPPED_VIA_BUILD` because a sibling lane moved main](2026-09-12-shipped-via-build-sibling-landing.md) | `finalizeOutcome` relabelled any SKIPPED verdict as SHIPPED_VIA_BUILD on a pre/post HEAD delta; in fleet mode a sibling's landing moves HEAD, so scout→triage→`triage-empty-commitment` (no ship) was credited as shipped — `IsTriageNoWorkResult` false, throughput credited, dossier PASS, loop breakers blind. Fix: the cycle's own ship latch, persisted as `CycleState.Shipped` and set by `latchShippedState` on BOTH dispatch roots (a first draft's in-memory field would have silently forwarded `false` on the resume root) | `core/cycle_outcome.go`, `core/cyclerun.go`, `core/cyclerun_postreview.go`, `core/resume_execution.go`, `cyclestate/state.go` | ✅ | `core/shipped_via_build_own_ship_test.go::TestRunCycle_EmptyTriageCommitmentSurvivesSiblingLanding` (cycle 1630's exact shape through the composed `RunCycle` path; RED on the old code on all three assertions) + `::TestCompleteCycle_ForwardsTheShipLatchToTheOutcomeLabel` (closeout reads the checkpoint latch, both rows) + `::TestRunCycle_ShipPassPersistsTheShipLatch` (fresh-root writer site) + `core/shipped_via_build_own_ship_integration_test.go::TestResumeLifecycle_ShipPassPersistsTheShipLatch` (resume-root writer site) + `::TestResumeLifecycle_PostShipSkippedVerdictReadsTheCheckpointLatch` (resume root labels by the checkpoint, both rows) + `cyclestate/shipped_latch_test.go::TestCycleState_ShippedRoundTripsAndIsOmittedWhenFalse` (persisted under `shipped`, omitted when false) + `core/orchestrator_outcome_test.go::TestFinalizeOutcome_SkippedWithoutOwnShip_NeverShippedViaBuild` / `::_SkippedWithOwnShip_ShippedViaBuild` / `::_OwnShipTrumpsAdvisory` (the latch table; the two HEAD-moved tests flipped); 6 mutants killed, each confirmed to build: fresh latch call dropped, resume latch call dropped, finalizeCycle reads false, latchShippedState sets nothing, latch→true, latch→false |
