...
| [cycle-1571 H3 — audit FAIL invisible to ship: binding fail-open across cycles](2026-08-26-audit-binding-fail-open.md) | FAIL verdict emitted no auditor binding + run-scope lookup miss silently bound a sibling lane's audit → observed: foreign HEAD_MOVED masks this run's FAIL, burns a re-audit slot; latent: FAILed cycle ships on sibling's PASS when HEADs coincide | `core/phase_bindings.go`, `phases/ship/audit.go` | ✅ | `core/phase_bindings_fail_verdict_test.go` (FAIL records binding, cache-Put refused with WARN vacuity control) + `phases/ship/runscope_test.go` (foreign-run + unstamped refusal, flipped fallback pin) — 3 mutants killed (2 by RED phase, cache-guard explicit) |
