...
See [domain-adapters.md](../domain-adapters.md) for what is implemented — since ADR-0099 slice 2 only `domain` has a Go reader (it sets the project's default deliverable kind); `evalMode`/`shipMechanism`/`buildIsolation` are legacy prompt-layer vocabulary.
...
When `models.json` exists, it takes precedence over auto-detection. When absent, the orchestrator uses the default mapping for the detected provider. See also [SKILL.md](../../skills/loop/SKILL.md) for dynamic model routing rules.
