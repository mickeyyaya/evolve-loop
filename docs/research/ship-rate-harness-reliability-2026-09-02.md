...
| R3 | **Deterministic floor before the judge** — re-targeted by forensics (2026-09-03): the build handoff floor already exists (`core/build_floor_reviewer.go`, E2 chain + correction ladder); the deterministic gate that was FAILing (explanation digest, 3/11) fired on a tree the AUDITOR had mutated, and 5/11 were the review section's FORMAT. Landed as the read-only phase worktree fence (`treefence` + `PhaseRequest.WorktreeReadOnly` + runner) and the tolerant explanation-review parser. | G5; buckets (a)(c)(d)(e) | M | landed — ADR-0097 |
| R4 | **Inline the contract** — acceptance criteria verbatim (from the bound inbox record, one source) + the ACS predicate names `go test -list` finds, as a harness-owned `## Task Contract` block in the tdd, build and audit prompts (`core.seedTaskContract`, both dispatch surfaces; the grader reads the same words). The `handoff-tdd.json` half was retired: the predicate inventory IS the handoff and is derived from the test files. | G3, G4 | M | landed — ADR-0098 |
