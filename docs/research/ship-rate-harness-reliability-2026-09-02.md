...
|---|---|---|---|---|
| **R1** | **Arm repair-round escalation** — plumb `AuditRepairAttempts` into the tier resolver so the declared `audit_retry_2plus → deep` fires; add `effort_overrides` with the same situation keys. | G1; the 3-round grind | S | implemented (this session, separate PR) |
| **R2** | **Repair brief carries the auditor's findings + reflection memory** — parse `### H1 (HIGH) — …` sections from the final report into the brief (HIGH/CRITICAL first, capped), name findings that persisted from the previous round, archive `build-prompt.round<N>.txt`/`tdd-prompt.round<N>.txt`. | G2, G10 | S–M | implemented (this session, separate PR) |
