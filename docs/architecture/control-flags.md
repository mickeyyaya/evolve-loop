...
| `EVOLVE_ADVISOR_DEPTH` | deprecated | int | — | Advisor Maximization (ADR-0052) | Replaced by WithDepthCheck(AdvisorDepthExceeded) DI seam on PhaseAdvisor (cycle-4 flag-reduction). No longer read in production env; check moved to injectable advisor_depth.go. |
| `EVOLVE_ANTHROPIC_BASE_URL` | deprecated | — | — | — | Replaced by policy.json bridge.anthropic_base_url (cycle-5 flag-reduction). Set BridgePolicy.AnthropicBaseURL instead; env read removed from driver_claudetmux.go and setup.go. |
...
| `EVOLVE_DISABLE_WORKSPACE_GUARD` | deprecated | — | — | — | Replaced by CycleRequest.DisableWorkspaceGuard bool field (cycle-4 flag-reduction). Bridged from env in cmd_cycle.go for backward compat. |
...
| `EVOLVE_HANG_CLASSIFIER` | deprecated | — | — | — | Replaced by policy.json classify.hang_classifier (cycle-5 flag-reduction). Set ClassifyPolicy.HangClassifier via policy.json; env read removed from cycleclassify.Classify, replaced with SetHangClassifier() from policy at startup. |
...
| `EVOLVE_MARKETPLACE_DIR` | deprecated | — | — | Observability / Prompt Tuning | Replaced by --marketplace-dir flag in marketplace-poll and marketplaceDir param in releasepipeline (cycle-4 flag-reduction). No longer read from env. |
...
| `EVOLVE_MODELCATALOG_AUTOREFRESH` | deprecated | — | — | — | Replaced by policy.json catalog.auto_refresh (cycle-5 flag-reduction). Set CatalogPolicy.AutoRefresh=false to disable; env read removed from cmd_models_live.go, shouldRefreshCatalog now takes bool from policy. |
...
| `EVOLVE_PLATFORM` | deprecated | — | — | Platform / CLI Hybrid | Replaced by Options.Platform field in detectcli and --platform flag in cmd_detect_cli.go (cycle-4 flag-reduction). No longer read in detectcli.go. |
| `EVOLVE_PLUGIN_ROOT` | active | — | — | Core Infrastructure (never consolidate) | Read-only plugin install location |
| `EVOLVE_POLICY_BYPASS` | deprecated | — | — | — | Replaced by PhaseRequest.BypassPolicy bool field (cycle-4 flag-reduction). Bridged from env in cmd_cycle.go for backward compat. |
...
| `EVOLVE_REAP_ORPHANS` | active | — | — | Concurrency / Sibling-Worktree (ADR-0054) | Operator opt-out for the Tier-3 liveness orphan session reaper (ADR-0054 Slice 3). Default 1 = active: sessionreaper.ReapOrphans runs in looppreflight and removes tmux sessions from SIGKILL'd runs whose .lease is stale. Set 0 to disable reaping for a run. Safety invariant: a run whose .lease is Fresh is NEVER reaped regardless of this flag; the lease is the sole liveness oracle. This flag is an operator convenience opt-out — it does NOT gate sessionreaper's core reap logic. |
