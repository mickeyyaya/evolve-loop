...
Normal Ship compares its current full snapshot to the sealed host tree as well as retaining the existing HEAD, precommit, and landed-tree checks. The plane-wide `git diff HEAD` check applies only when the plane is the tree Ship lands from: a worktree ship is bound by the snapshot of that worktree, and the plane's tracked bookkeeping (the inbox queue) is not part of what it commits (2026-09-26, cycle 1701). The new mismatch is `AUDIT_BINDING_TREE_MISMATCH`, a precondition handled by the existing re-audit recovery route. Binary normalization must happen before Audit. Normalization or source changes after Audit require fresh execution.
