...
- **Every plan is validated, and no record erases another (cycle 1155):** `ValidatePlan` runs on the RAW plan of *both* decisions — the upfront plan (`core/cyclerun.go`) and each post-scout re-plan (`core/cyclerun_replan.go`, pre-clamp, so the advisor's intent is what gets recorded; validating the clamped plan would record nothing, since the clamp has already dropped the unknown phase). Findings land one-artifact-per-decision, mirroring `phase-<kind>.json`: `advisor-rejections.json` for the upfront plan and `advisor-rejections-replan-<n>.json` for the n-th re-plan, so accumulation under `EVOLVE_ROUTER_REPLAN_DEPTH > 1` needs no read-modify-write of a shared file and a later record can never overwrite an earlier one. All kinds keep the `plan_rejections` ledger kind; `ArtifactPath` distinguishes them. Report-only throughout — the clamp remains the sole disposer.
