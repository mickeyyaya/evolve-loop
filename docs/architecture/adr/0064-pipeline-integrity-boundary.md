...
- **Amendment 2026-09-26 (F37, cycle 1689) — P2's question asked where it can be acted on.** Cycle 1689's builder rewrote the protected `go/internal/core/cyclerun.go` through a shell tool. P1 never saw it, and nothing would have looked again until P2 at ship, after a full audit. P2's refusal would then have recovered into a **re-audit** of the same diff (the cycle-230 audit↔ship loop) until the budget aborted the cycle. The operator stopped the lane before ship.
  - **The build handoff floor asks P2's question** (`core.ProtectedSurfaceFloorChecks`). The one floor composition, `productionBuildFloorChecks` in the protected `cmd/evolve/cmd_cycle_config.go`, runs it FIRST, then the deterministic engine; the cycle reviewer and `evolve selfcheck build` both run it. Every changed path on the manifest is a floor failure, judged over the cycle-base diff plus untracked files. That is the set P2 judges once the post-record soft-reset puts HEAD back at the cycle base. So the correction ladder has the builder restore the file in-phase, before audit is spent. `guards.IsProtectedSurface` is injected at the composition root because `guards` imports `core`.
  - **Rename detection is OFF on both sides.** `git diff --name-only` with renames detected prints only a rename's new path, so a protected file moved to an unprotected name, and weakened while it stays over 50% similar, passed P2 too.
  - **P2 stays the backstop, and its refusal now recovers into BUILD** (`router` `control-plane-rebuild`, ordered before `precondition-reaudit`, within the constant ship-recovery budget of 2). A re-audit re-verifies the same diff, and the debugger cannot change what a cycle may write; the builder can, and on re-entry the floor names the path. Ship passes the builder nothing else.
  - Record: [2026-09-26-lane-edited-the-control-plane-through-a-shell-tool](../../incidents/2026-09-26-lane-edited-the-control-plane-through-a-shell-tool.md).
