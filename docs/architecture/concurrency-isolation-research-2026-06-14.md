...
1. **Stale-lock liveness** on the single global ledger/ship.lock — a serialized writer is a liveness SPOF (cf. Git's too-short 1s packed-refs timeout). Need an explicit lock timeout + crashed-holder recovery (the CA.1 flock appears blocking with no documented timeout — verify). *This is the hard prerequisite for the deferred `mergequeue.lock` (S5b): a lock held across a recovering cycle's multi-minute re-audit without timeout/crashed-holder recovery turns a bounded abort into an unbounded-deadlock SPOF. Full deferral rationale + trip-wire: [ADR-0049 → Implementation status → Deferred: mergequeue.lock](adr/0049-concurrent-multi-cycle-execution.md).*
