...
| **P2** | **v9.0.4 / v10.10.0 — builder turn-budget guidance** (checkpoint at turn 15, hard-exit at turn 20) | **~$0.10–$0.20/cycle** (saves 3–5 turns × $0.034/turn at current baseline) | Medium | **Shipped cycle 70** — `builder.json:turn_budget_guidance`, `agents/evolve-builder.md` §"Budget Checkpoint Protocol"; **INERT cycle 72** — C71 artifact: 39 turns / $0.7305 vs C69 baseline 26 turns / $0.5931 = +50% turns, +23% cost. Exceeds scout falsification threshold (>25 turns). Advisory-only turn_budget_guidance cannot constrain the implementer that produces the telemetry (consecutive falsification C70+C71 per [[cycle-71-builder-estimate-vs-artifact]]). Case A (programmatic kill) deferred — no native --max-turns flag in claude -p. | This doc §"Per-phase"; `docs/operations/incidents/turn-overrun-c69.md`; `docs/architecture/adr/0009-p2-turn-budget-inert.md` |
| P3 | Triage bedrock right-sizing (shorter persona + slimmer context) | ~$0.10 (40% of triage) | Low | Identified | D2 above |
| P4 | Auditor anchor mode for intent.md (`intent.md:acceptance_criteria`) | ~$0.03–$0.06/cycle (7.3 KB × 22 turns × Opus cache-read rate) | **Tiny** (1 JSON edit) | **Shipped cycle 69; INERT** — input delta C68→C70: −6.1% (−58,014 tokens); cost delta: **+17.6% (+$0.1651)** — input savings swamped by 66% output-token increase at 5× price ratio. See `knowledge-base/research/anchored-context-injection-2026.md` | D1 above |
