...
5. **Mutation-testing pre-flight** (v8.20+): runs `scripts/mutate-eval.sh` against new evals; kill rate < 0.8 -> flagged as tautological
...
**Script**: `scripts/ship.sh` — privileged shell, NOT a subagent. Allowlisted as the canonical entry point by `ship-gate.sh`.
...
**Script**: `scripts/record-failure-to-state.sh` (privileged shell).
...
The classification taxonomy (`scripts/failure-classifications.sh`):
...
The `INDEX.md` at the directory root is auto-regenerated by `scripts/index-investigations.sh` after each merge. Operator workflow:
...
`prev_hash` chains entries; modifying entry N breaks every entry after. `.evolve/ledger.tip` records the latest entry's full SHA — truncation detection. `bash scripts/verify-ledger-chain.sh` walks the chain and reports breaks (rc=1) or truncation (rc=2).
...
`scripts/failure-adapter.sh` reads `state.json:failedApproaches[]` and emits a deterministic decision JSON:
...
- v8.35.0: adaptive auditor model selection via `scripts/diff-complexity.sh` — Sonnet for trivial diffs, Opus for complex/security
