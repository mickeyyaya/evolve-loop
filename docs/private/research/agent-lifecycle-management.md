...
| **Deploy** | `legacy/scripts/publish.sh` | Published skill/agent, deployment record | Ship agent to production |
| **Monitor** | Operator, `legacy/scripts/observability/cycle-health-check.sh` | Health reports, drift alerts | Track quality, cost, drift continuously |
...
| Develop -> Test | All unit tests pass; code review complete | Builder phase gate (`legacy/scripts/lifecycle/phase-gate.sh`) |
| Test -> Deploy | Eval scores meet baseline; no safety violations | Auditor phase gate (`legacy/scripts/lifecycle/phase-gate.sh`) |
...
| **Manual Lifecycle Management** | Error-prone; stages skipped under time pressure; no audit trail | Automate stage gates with `legacy/scripts/lifecycle/phase-gate.sh`; enforce via CI |
