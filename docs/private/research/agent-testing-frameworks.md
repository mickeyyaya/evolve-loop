...
| **Integration** | `phase-gate.sh` | Validate phase transitions: Scout → Build → Audit handoffs produce required artifacts | `legacy/scripts/lifecycle/phase-gate.sh` |
| **E2E** | `cycle-health-check.sh` | Verify full cycle completion: all reports exist, no integrity violations, metrics recorded | `legacy/scripts/observability/cycle-health-check.sh` |
