...
### Agent Mailbox
Each agent can leave a structured message for a downstream agent by appending to `workspace/agent-mailbox.md` (Markdown table with columns: `from`, `to`, `type`, `cycle`, `persistent`, `message`). The downstream agent reads mailbox entries addressed to it at the start of its phase. Messages with `persistent: true` survive Phase 4 cleanup across cycles; non-persistent messages are cleared each cycle. This gives inter-agent communication a durable, inspectable channel.
