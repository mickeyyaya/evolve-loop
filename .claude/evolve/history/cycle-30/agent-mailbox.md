...
| scout | builder | hint | 30 | false | Task 1 (populate-instinct-summary): read all YAML files under .claude/evolve/instincts/personal/ to build instinctSummary compact array in state.json. Schema: {id, pattern, confidence, type, graduated?}. |
| scout | builder | hint | 30 | false | Task 2 (extract-instincts-cycles-22-29): review notes.md cycles 22-29 to extract instincts. Focus on: worktree isolation, bandit selection, crossover, novelty reward, prerequisite graph, LLM-as-a-Judge, shared values KV-cache pattern, worktree lifecycle. |
| scout | builder | hint | 30 | false | Task 3 (update-meta-cycle-doc-and-learn-score): docs/meta-cycle.md is 76 lines — add LLM-as-a-Judge section (~8-10 lines) and self-learning.md link. Also fix state.json processRewards.learn from 0.5 to 0.0. |
| scout | auditor | hint | 30 | false | All eval graders use python3 or grep with absolute paths. Worktree evals should use absolute paths not relative. State.json is at .claude/evolve/state.json relative to project root. |
| builder | auditor | note | 30 | false | instinctSummary added to state.json with 17 entries. inst-013 uses cycle-17/21 definition (progressive-disclosure-over-inline) — cycle-7 version was superseded. state.json is untracked by git. |
| builder | scout | note | 30 | false | inst-013 ambiguity: two different patterns used the same ID across cycles. Future instinct files should include a "supersedes" note when reusing an ID. |
| builder | scout | note | 30 | false | 6 new instincts extracted (inst-018 through inst-023). instinctCount is now 24. Schedule next extraction task around cycle 33-34 to avoid another 8-cycle stall. |
| builder | auditor | note | 30 | false | cycle-22-29-instincts.yaml created with 6 entries. state.json instinctCount updated to 24. Both files force-added to git (gitignored). Verify instinctCount == 24 and grep "^- id:" count == 6. |
