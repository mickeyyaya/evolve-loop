...
AGENT_REFS=$(grep -oE 'evolve-[a-z]+\.md' skills/loop/SKILL.md | sort -u)
MISSING_AGENTS=0
...
PHASES_FIELDS=$(grep -oE 'state\.json\.?[a-zA-Z]+' skills/loop/phases.md 2>/dev/null | sed 's/state\.json\.*//' | sort -u | head -20)
PROTO_FIELDS=$(grep -oE '"[a-zA-Z]+"' skills/loop/memory-protocol.md 2>/dev/null | tr -d '"' | sort -u)
MISSING_FIELDS=0
...
PHASE_NUMS=$(grep -oE 'Phase [0-9]+' skills/loop/phases.md | grep -oE '[0-9]+' | sort -n | uniq)
...
PROTO_FILES=$(grep -oE '[a-z]+-[a-z]+\.md' skills/loop/memory-protocol.md | sort -u)
PHASE_FILES=$(grep -oE 'workspace/[a-z]+-[a-z]+\.md' skills/loop/phases.md | sed 's|workspace/||' | sort -u)
...
# Searches span skills/loop/ since content was split across files

# 1. Skill files contain rollback/revert instructions
ROLLBACK=$(grep -rci -e 'rollback' -e 'revert' -e 'git revert' skills/loop/ 2>/dev/null | awk -F: '{s+=$NF} END{print s+0}')
ROLLBACK_SCORE=$((ROLLBACK >= 3 ? 100 : (ROLLBACK >= 1 ? 50 : 0)))

# 2. HALT protocol exists in operator and skill files
HALT_SKILLS=$(grep -rc 'HALT' skills/loop/ 2>/dev/null | awk -F: '{s+=$NF} END{print s+0}')
...
RETRY=$(grep -rci -e 'max.*3' -e '3.*attempt' -e 'max.*iteration' skills/loop/ 2>/dev/null | awk -F: '{s+=$NF} END{print s+0}')
RETRY_SCORE=$((RETRY >= 2 ? 100 : (RETRY >= 1 ? 50 : 0)))

# 4. Worktree cleanup is enforced
CLEANUP=$(grep -rci -e 'worktree.*cleanup' -e 'worktree.*prune' -e 'worktree.*remove' skills/loop/ 2>/dev/null | awk -F: '{s+=$NF} END{print s+0}')
CLEANUP_SCORE=$((CLEANUP >= 2 ? 100 : (CLEANUP >= 1 ? 50 : 0)))

# 5. Input validation at system boundaries
VALIDATION=$(grep -rci 'validat' skills/loop/ 2>/dev/null | awk -F: '{s+=$NF} END{print s+0}')
...
EVAL_RUNNER=$(test -f skills/loop/eval-runner.md && echo 100 || echo 0)

# 2. Eval definition format documented with all 3 sections
EVAL_SECTIONS=$(grep -c '## Code Graders\|## Regression Evals\|## Acceptance Checks' skills/loop/eval-runner.md 2>/dev/null || true)
EVAL_SECTIONS=${EVAL_SECTIONS:-0}
EVAL_FORMAT_SCORE=$((EVAL_SECTIONS >= 3 ? 100 : (EVAL_SECTIONS * 33)))

# 3. Eval checksum mechanism exists
CHECKSUM=$(grep -rc -e 'checksum' -e 'sha256' skills/loop/ 2>/dev/null | awk -F: '{s+=$NF} END{print s+0}')
CHECKSUM_SCORE=$((CHECKSUM >= 2 ? 100 : (CHECKSUM >= 1 ? 50 : 0)))

# 4. Eval tamper detection documented
TAMPER=$(grep -rci 'tamper' skills/loop/ 2>/dev/null | awk -F: '{s+=$NF} END{print s+0}')
TAMPER_SCORE=$((TAMPER >= 2 ? 100 : (TAMPER >= 1 ? 50 : 0)))

# 5. Meta-cycle mutation testing documented
MUTATION=$(grep -rci 'mutation' skills/loop/ 2>/dev/null | awk -F: '{s+=$NF} END{print s+0}')
...
PHASES_LINES=$(wc -l < skills/loop/phases.md | tr -d ' ')
...
SKILL_FILES=$(find skills/loop/ -name "*.md" | wc -l | tr -d ' ')
...
INIT_FIELDS=$(grep -oE '"[a-zA-Z]+"' skills/loop/SKILL.md | head -40 | tr -d '"' | sort -u)
PROTO_SCHEMA=$(grep -oE '"[a-zA-Z]+"' skills/loop/memory-protocol.md | tr -d '"' | sort -u)
INIT_MISSING=0
...
DUP_FIELDS=$(grep -oE '^ *"[a-zA-Z]+"' skills/loop/memory-protocol.md | sort | uniq -d | wc -l | tr -d ' ')
DUP_SCORE=$((DUP_FIELDS == 0 ? 100 : (100 - DUP_FIELDS * 20)))

# 3. JSON examples syntactically valid (spot check)
JSON_BLOCKS=$(grep -c '```json' skills/loop/memory-protocol.md || true)
JSON_BLOCKS=${JSON_BLOCKS:-0}
JSON_SCORE=$((JSON_BLOCKS >= 3 ? 100 : (JSON_BLOCKS >= 1 ? 75 : 50)))

# 4. Layer numbering is sequential
LAYERS=$(grep -oE 'Layer [0-9]+' skills/loop/memory-protocol.md | grep -oE '[0-9]+' | sort -n)
...
NON_KEBAB=$(find skills/loop/ -name "*.md" ! -name "SKILL.md" | grep '[A-Z_]' | wc -l | tr -d ' ')
...
DIR_SCORE=$(test -d skills/loop -a -d agents -a -d docs && echo 100 || echo 50)
...
PHASES_DEFINED=$(grep -cE '### Phase [0-9]+:' skills/loop/phases.md || true)
...
MISSING=0
for a in $(grep -oE 'evolve-[a-z]+\.md' skills/loop/SKILL.md | sort -u | tr '\n' ' '); do
  test -f "agents/$a" || MISSING=$((MISSING + 1))
done
AGENT_COV_SCORE=$((MISSING == 0 ? 100 : (100 - MISSING * 25)))

# 3. All referenced support files exist
SUPPORT_FILES="skills/loop/eval-runner.md skills/loop/memory-protocol.md skills/loop/phases.md"
SUP_MISSING=0
