...
| Plan-mode dispatch (per-phase) | `EVOLVE_<PHASE>_PERMISSION_MODE` | unset (v12.1+) | Sets the Claude Code `--permission-mode` flag for a single phase invocation. Values per Claude Code: `plan`, `acceptEdits`, `default`. Wired in `go/internal/phases/<phase>/<phase>.go::resolveExtraFlags` (build phase shipped first as proof-of-pattern in v12.1.0; other phases replicate mechanically). Precedence: env > `profile.permission_mode` (JSON field in `.evolve/profiles/<phase>.json`) > unset. The flag is appended to `BridgeRequest.ExtraFlags`; only Claude drivers honor it (claude-p, claude-tmux). Codex/Gemini drivers refuse it gracefully (`plan_mode_degraded` ledger entry). |
