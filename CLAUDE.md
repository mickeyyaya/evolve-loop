...
- Routing: `EVOLVE_DYNAMIC_ROUTING=advisory` default (since 2026-06-06, retro steps 1-3 landed; `=off` is the static escape hatch); integrity floor `ship ⇒ build ∧ audit ∧ (tdd unless trivial)`; policy pins live in `.evolve/policy.json` (`pins`; the checked-in file pins memo → agy/fast; policy bypass is off by default). Swarm: stage=shadow — a **compiled default**, overridable via `.evolve/policy.json` `swarm.stage` (not set in the checked-in file).
